...
def test_memory_layer_config_defaults(tmp_path: Path, monkeypatch) -> None:
    ...
    assert config.context.max_items == 12
    assert config.scoring.recency_half_life_seconds == 1800
    assert config.pipeline.extractor == "rule"
    assert config.dreaming.enabled is False
    assert config.time.timezone == "UTC"
    assert config.time.timestamp_format == "%Y-%m-%d %H:%M"
...
def test_memory_config_toml_overrides(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(memory_config, "get_data_dir", lambda: tmp_path)
    ...
    config = memory_config.load_memory_config(path=tmp_path / "memory.yaml")
    assert config.recall.top_k == 3
    assert config.context.max_items == 4


def test_memory_time_config_overrides(tmp_path: Path, monkeypatch) -> None:
    monkeypatch.setattr(memory_config, "get_data_dir", lambda: tmp_path)
    ...
    assert config.time.timezone == "UTC"
    assert config.time.timestamp_format == "%Y-%m-%d"
