...
- 记忆层切到可配置后端（SQLite/StateStore），并引入 BM25 + 向量检索 + KG 的混合召回与 RRF 融合策略。
- 对话引擎在上下文构建阶段注入召回记忆，形成独立的系统提示片段，避免污染会话历史。
- 记忆层新增 TTL 与压缩机制，保持长期记忆可控并便于热启动恢复索引。
- 情感层抽出配置管理器，支持插件式规则/词典注册、热加载与 per-character 覆盖。
- 关联召回通过实体扩展与 KG 关联检索触发二跳扩散，并在召回不足时回退最近记忆。
